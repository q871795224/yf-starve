...
Clip 旁边的“组合动作”下拉框会把相关片段按顺序播放。例如官方骑乘资源里有完整战吼、牛攻击、骑手单独攻击、吃草、犁地步行和奔跑；蓄力冲撞资源会按 `lancecharge_pre → lag_pre → loop → lag → pst` 播放。官方 `atk_*` 是牛攻击并保持骑手坐姿，`player_atk_*` 是骑手单独攻击。两者的空间根、牛身姿态和骑手朝向都不同，不能直接按图层拼成同步攻击；要实现同步，需要重新制作共享根节点的动画资源。选择线性组合后，左右方向键也会跨越片段切换。
