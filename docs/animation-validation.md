...
Clip 旁边的“组合动作”下拉框会把相关片段按顺序播放。例如官方骑乘资源里有完整战吼、牛攻击、骑手单独攻击、吃草、犁地步行和奔跑；蓄力冲撞资源会按 `lancecharge_pre → lag_pre → loop → lag → pst` 播放。官方 `atk_*` 是牛攻击并保持骑手坐姿，`player_atk_*` 是骑手单独攻击。下拉框里的“牛 + 骑手同步攻击”是一个离线并行组合：从 `atk_*` 取 Beefalo 图层，再从 `player_atk_*` 取骑手图层，按同一时间轴叠加；它不是官方提供的单个 Clip。选择线性组合后，左右方向键也会跨越片段切换；并行组合按合成后的总帧数逐帧查看。
