...
Clip 旁边的“组合动作”下拉框会把相关片段按顺序播放。例如官方骑乘资源里有完整战吼、牛攻击、骑手攻击、吃草、犁地步行和奔跑；蓄力冲撞资源会按 `lancecharge_pre → lag_pre → loop → lag → pst` 播放。官方 `atk_*` 帧自身已经包含 `player_hand`、`ARM_normal` 等骑手图层，`player_atk_*` 则是另一套骑手攻击片段。选择组合后，左右方向键也会跨越片段切换。
